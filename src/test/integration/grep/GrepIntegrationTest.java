...
public class GrepIntegrationTest {
	...
	public void testIntegrateCat() throws AbstractApplicationException, ShellException {
		...
		String expected = Environment.getAbsPath(RELATIVE_TOTO1)
				+ ": The moonlit wings reflect the stars that guide me towards salvation" + LINE_SEPARATOR
				+ Environment.getAbsPath(RELATIVE_TOTO2) + ": There's nothing that a hundred men or more could ever do"
				+ LINE_SEPARATOR + Environment.getAbsPath(RELATIVE_STRANGER1) + ": If you say that you are mine"
				+ LINE_SEPARATOR;
		String output = shell.parseAndEvaluate(command);
		assertEquals(expected, output);
	}
	...
	public void testIntegrateTail() throws AbstractApplicationException, ShellException {
		...
		String expected = Environment.getAbsPath(RELATIVE_TOTO2) + ": It's gonna take a lot to drag me away from you"
				+ LINE_SEPARATOR;
		String output = shell.parseAndEvaluate(command);
		assertEquals(expected, output);
	}
	...
	public void testIntegrateCal() throws AbstractApplicationException, ShellException, IOException {
		...
		String expected = "Never gonna give you up" + LINE_SEPARATOR + "Never gonna let you down" + LINE_SEPARATOR
				+ "Never gonna run around and desert you" + LINE_SEPARATOR + "Never gonna make you cry" + LINE_SEPARATOR
				+ "Never gonna say goodbye" + LINE_SEPARATOR + "Never gonna tell a lie and hurt you" + LINE_SEPARATOR;
		expected = expected + new String(Files.readAllBytes(Paths.get(RELATIVE_EXP_DIR + "currentMonth")))
				+ LINE_SEPARATOR;
		String output = shell.parseAndEvaluate(command);
		assertEquals(expected, output);
	}
	...
	public void testIntegrateSed() throws AbstractApplicationException, ShellException {
		...
		String expected = Environment.getAbsPath(RELATIVE_TOTO1)
				+ ": The moonlit wings reflect the stars this guide me towards salvation" + LINE_SEPARATOR
				+ Environment.getAbsPath(RELATIVE_TOTO2) + ": There's nothing this a hundred men or more could ever do"
				+ LINE_SEPARATOR + Environment.getAbsPath(RELATIVE_STRANGER1) + ": If you say this you are mine"
				+ LINE_SEPARATOR;
		String output = shell.parseAndEvaluate(command);
		assertEquals(expected, output);
	}
