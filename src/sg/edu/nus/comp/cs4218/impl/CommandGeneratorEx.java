...
public class CommandGeneratorEx {
    ...
    public CommandGeneratorEx( Globber cmdGlobber, List<Segment> cmdSegments ) {
            ...
            if(firstSymbol == Symbol.UNQUOTED_UNRELATED ){
                ...
                while(index < segmentSymbols.length()){
                    if(segmentSymbols.charAt(index) != Symbol.UNQUOTED_UNRELATED){
                        ...
                        // If we allow "echo"hi there -> prints out hi there
                        //currentCmd.setAppName(segment.substring(0, index));
                        //currentCmd.addArguments(segment.substring(index, segment.length()));
                        //return;
                        
                        // If we do NOT allow "echo"hi there -> throws invalid exception
                        throw new ShellException(Shell.EXP_SYNTAX);
                    }
                    ++index;
                }
