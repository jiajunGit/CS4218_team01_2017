...
/**
 * A class used to generate command objects to be used later by shellImpl for
...
public class CommandGenerator {
	...
	/**
	 * Generate command objects using the list of segments provided when
	 * initializing this class's constructor
	 * 
	 * @return List of command objects
	 * 
	 * @throws ShellException
	 *             If an exception happens while generating command objects from
	 *             the list of segments provided.
	 * 
	 */
		...
		for (Segment segment : segments) {
			...
			if (!segment.hasGlob()) {
				...
				if (globs.length > 0) {
					...
					currentStage = populateCommand(segment.getSegment(), segment.getSegmentSymbol(), currentStage,
							false);
				}
			}
		}
	...
	private Stage populateCommandForNormal(String segment, String segmentSymbols, char symbol) throws ShellException {
	...
	// This method assumes segment to be string with all special symbols
	// disabled
	private Stage populateCommand(String segment, Stage stage, boolean isGlobbedArgs) throws ShellException {
