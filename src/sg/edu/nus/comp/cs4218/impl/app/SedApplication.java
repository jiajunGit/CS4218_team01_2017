 ...
 * <dl>
 * <dt>REPLACEMENT</dt>
 * <dd>specifies replacement rule, as follows: (1) s/regexp/replacement/ �
 * replace the first (in each line) substring matched by regexp with the string
 * replacement. (2) s/regexp/replacement/g � replace all the substrings matched
 * by regexp with the string replacement. Note that the symbols �/� used to
 * separate regexp and replacement string can be substituted by any other
 * symbols. For example, �s/a/b/� and �s|a|b|� are the same replacement rules.
...
public class SedApplication implements Sed {
	 ...
	 *            rule is as follows: (1) s/regexp/replacement/ � replace the
	 *            first (in each line) substring matched by regexp with the
	 *            string replacement. (2) s/regexp/replacement/g � replace all
	 *            the substrings matched by regexp with the string replacement.
	 *            Note that the symbols �/� used to separate regexp and
	 *            replacement string can be substituted by any other symbols.
	 *            For example, �s/a/b/� and �s|a|b|� are the same replacement
